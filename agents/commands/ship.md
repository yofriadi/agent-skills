...
Read and follow the `skill://shipping-and-launch` skill.
...
Spawn three subagents concurrently using the `task` tool. **Issue all three `task` tool calls in a single assistant turn so they execute in parallel** — sequential calls defeat the purpose of this command.

Pass the corresponding specialist agent type name:
...
**Persona resolution.** The custom agent definitions (`code-reviewer`, `security-auditor`, or `test-engineer`) are located in `agents/` of this workspace and will be discovered automatically.
