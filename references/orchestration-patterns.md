...
**Examples in this repo:** `/review`, `/test`, `/refactor`.
...
**Use a built-in `Explore` subagent** when available rather than defining a custom research persona. An Explore-style subagent runs on a smaller model, is denied write/edit tools, and is purpose-built for this pattern. Define a custom research subagent only when it doesn't fit (e.g. you need a domain-specific system prompt the model wouldn't infer).
